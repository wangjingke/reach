...
win=c("2h", "25h", "3h", "4h", "6h", "1d")
distance=c(0, 0.25, 0.5, 1, 2)
...
for (i in 1:nrow(common)) {
    ...
    if (nrow(dietX)>0 & nrow(emaX)>0) {
        outputX=cbind(emaX, matrix(NA, nrow=nrow(emaX), ncol=72, dimnames=list(c(), c(ema.diet, paste0(ema.diet, "_count"), paste0(ema.diet, "_totalmeal")))))
...
# for (i in 1:nrow(common)) {
#     dietX=diet[diet$cpartid==common$uid[i],]
#     emaX=w1[w1$ID==common$ID[i] & w1$DATE %in% unique(dietX$dintake) & w1$COMPLY==1,]
#     if (nrow(dietX)>0 & nrow(emaX)>0) {
#         outputX=cbind(emaX, matrix(NA, nrow=nrow(emaX), ncol=28, dimnames=list(c(), ema.diet)))
#
#         for (j in 1:nrow(outputX)) {
#             for (k in 1:3) {
#                 win.upper=outputX$response_time[j]-hrs(k+1)
#                 win.lower=outputX$response_time[j]+hrs(k-1)
#                 outputX[j, paste0(diet.var, "_", win[k])]=sapply(dietX[dietX$time>win.upper & dietX$time<win.lower, c(diet.var)], eaten)
#             }
#             outputX[j, paste0(diet.var, "_1d")]=sapply(dietX[dietX$dintake==outputX[j,"DATE"], c(diet.var)], eaten)
#         }
#         output=rbind(output, outputX)
#     }
# }
